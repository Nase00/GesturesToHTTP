...
namespace Microsoft.Samples.Kinect.DiscreteGestureBasics
{
    ...
    public sealed class GestureResultView : INotifyPropertyChanged
    {
        ...
        private static readonly JObject requests = JObject.Parse(File.ReadAllText(@"requests.json"));
        ...
        public void UpdateGestureResult(bool isBodyTrackingIdValid, bool isGestureDetected, float detectionConfidence)
        {
            ...
            else
            {
                ...
                if (this.Detected)
                {
                    ...
                    TimeSpan InvokeDelay = TimeSpan.FromMilliseconds(3000);
                    {
                        if (detectionConfidence > 0.1 && DateTime.Now - this.LastInvoked > InvokeDelay)
                        {
        ...
        {
            JToken Request = (JToken)Requests["1"];
            JToken RequestHeaders = (JToken)Requests["1"]["options"]["headers"];
            JToken RequestBody = (JToken)Requests["1"]["body"];
            WebRequest request = WebRequest.Create(Request["options"]["path"].ToString());
            request.Method = "POST";

            foreach (var headerProp in RequestHeaders) {
                var key = ((JProperty)(headerProp)).Name;
                var value = ((JProperty)(headerProp)).Value;

                if (key == "Content-Type") {
                    request.ContentType = "application/json";
                } else {
                    request.Headers[key] = RequestHeaders[key].ToString();
                }
            }
